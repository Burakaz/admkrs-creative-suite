...
def product_gfx(tone):
    if tone == 'hydra':
        return '<div class="gfx bottle"><i class="cap"></i><i class="lbl">NOVA</i></div>'
    if tone == 'box':
        return '<div class="gfx pack"><i class="lbl">NOVA</i><i class="sub2">STARTER BOX</i></div>'
    return '<div class="gfx can"><i class="band"></i><i class="lbl">NOVA</i></div>'
...
def r_product(s):
    return f'<div class="sc-prod"><div class="hk sm a1">{E(s["hook"])}</div>{product_gfx(s["tone"])}' + \
...
def r_ui(s):
    ...
    if k == 'notes':
        return f'<div class="sc-ui ui-notes"><div class="ui-bar">Notizen</div><div class="ui-body"><div class="nhk">{E(s["hook"])}</div>{cap}</div></div>'
    if k == 'tweet':
        return f'<div class="sc-ui ui-tweet"><div class="tw-head">{avatar("N")}<div><b>{lab or "nova"}</b><span>@nova.nutrition</span></div></div><div class="tw-txt">{E(s["hook"])}</div><div class="tw-meta a3">♡ 2.418 · ↺ 312</div></div>'
    if k == 'chat':
        return f'<div class="sc-ui ui-chat"><div class="ui-bar">{lab or "Mara"}</div><div class="ui-body"><div class="bub in a1">{E(s["hook"])}</div><div class="bub out a2">{E(s.get("sub") or s.get("caption") or "")}</div></div></div>'
    if k == 'reddit':
    ...
    if k == 'search':
        return f'<div class="sc-ui ui-search"><div class="se-box"><i>🔍</i><span class="typing">{E(s["hook"])}</span></div><div class="se-sug a2">{E(s.get("sub") or "")}</div></div>'
    if k == 'email':
        return f'<div class="sc-ui ui-email"><div class="ui-bar">Posteingang</div><div class="ui-body"><div class="em-from">NOVA <span>09:41</span></div><div class="em-subj">{E(s["hook"])}</div><div class="em-prev">{E(s.get("sub") or "")}</div></div></div>'
    if k == 'calendar':
        return f'<div class="sc-ui ui-cal"><div class="ui-bar">Erinnerung</div><div class="ui-body"><div class="cal-dot a1"></div><div class="nhk">{E(s["hook"])}</div>{cap}</div></div>'
    if k == 'poll':
    ...
    if k == 'playlist':
        return f'<div class="sc-ui ui-play"><div class="pl-art">▶</div><div class="nhk">{E(s["hook"])}</div><div class="em-prev">{E(s.get("sub") or "NOVA · Mix")}</div><div class="pl-bar"><i></i></div></div>'
    if k == 'dating':
    ...
    if k == 'maps':
        return f'<div class="sc-ui ui-maps"><div class="mp"></div><div class="pin a1">📍</div><div class="nhk">{E(s["hook"])}</div>{cap}</div>'
    # comment
...
def r_talkinghead(s):
    return f'<div class="sc-vid th"><div class="person"><i class="head"></i><i class="body"></i></div>' + \
        (f'<div class="vtag">{E(s.get("label") or "")}</div>' if s.get('label') else '') + \
        f'<div class="cap-line">{words(s.get("caption") or s["hook"])}</div><div class="prog"><i></i></div></div>'

def r_broll(s):
    return f'<div class="sc-vid br"><div class="clips"><i class="c1"></i><i class="c2">{product_gfx(s["tone"])}</i><i class="c3"></i></div>' + \
        f'<div class="cap-line">{words(s.get("caption") or s["hook"])}</div><div class="prog"><i></i></div></div>'
...
def r_meme(s):
    return f'<div class="sc-meme"><div class="m-top">{E(s["hook"])}</div><div class="m-img">{product_gfx(s["tone"])}</div>' + \
...
def r_collage(s):
    it = (s.get('items') or ['', '', ''])[:3]
    tiles = f'<div class="tile t1">{product_gfx(s["tone"])}</div>' + ''.join(f'<div class="tile t{i+2}"><span>{E(x)}</span></div>' for i, x in enumerate(it[:3]))
...
def preview(style, medium):
    ...
    inner = ARCH[s['arch']](s)
    return f'<div class="pv t-{s["tone"]} an-{s["anim"]}" data-arch="{s["arch"]}"><div class="safe">{inner}</div><div class="pv-ui"><span class="pv-brand">NOVA</span><span class="pv-916">9:16</span></div></div>'
preview.missing = []
