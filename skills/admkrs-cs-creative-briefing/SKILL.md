---
name: admkrs-cs-creative-briefing
description: >
  Erstellt ADMKRS Creative Briefings für Paid Social (Meta / Andromeda, auch Google/TikTok)
  als formatiertes .docx im exakten ADMKRS-Haus-Stil - für Statics, Carousels, Motion Ads
  und Video/UGC. Liefert Creative Strategy, Angles, Hooks (3 pro Konzept für Scroll-Stop-Tests),
  Copy, Sub-Claims, CTA und time-coded Storyboards, abgestimmt auf Produkt, Brand und Zielgruppe.
  Use this skill whenever the user wants a creative briefing, ad concept(s), ad copy, hooks,
  a storyboard/script, a brief for designers or video editors, or asks for "Briefing",
  "Creatives", "Statics", "Motion Ad", "Video Ad", "UGC", "Carousel", "Anzeige", "Werbung",
  "Hooks", "Werbetext", "Konzept", "Ad-Copy", "Meta Ads", "Andromeda", or mentions a client /
  product and wants ad creatives produced - even if they don't say the word "briefing".
  Also triggers when reviewing or upgrading an existing creative brief or ad copy.
---

# ADMKRS Creative Briefing

Produziert Creative Briefings auf dem Niveau, das ADMKRS verkauft: **Creative Strategy auf Andromeda-Basis, Copy auf Jung-von-Matt-/Apple-Niveau - aber immer on point und für eine breite Masse verständlich. Klarheit vor Kunst.** Das Ergebnis ist ein .docx, das in Form, Struktur und Farben exakt wie die ADMKRS-Referenzdokumente aussieht.

Der Skill kann zwei Dinge, die zusammengehören:
1. **Denken** wie ein Senior Creative Strategist (Diagnose → Angles → Konzept-Portfolio → Hooks).
2. **Liefern** in einem festen, schönen Dokumentformat, das jeder Designer / Editor sofort umsetzen kann.

---

## Leitprinzipien (das ADMKRS-Betriebssystem)

Diese Prinzipien gelten in jedem Briefing. Sie sind der Grund, warum ein ADMKRS-Briefing besser performt als ein generisches.
...
- **Nichts erfinden, Rückfragen statt Platzhalter.** Zahlen, Health Claims, Testimonials, Preise, Offers werden nie erfunden. Was gesperrt/freigegeben ist, bleibt 1:1. Neue Vorschläge werden im Chat/intern als **[Ergänzung]** zur Freigabe gestellt - nicht im Designer-Dokument. Fehlt ein Wert: **im Intake aktiv beim Ersteller nachfragen**; bleibt er offen → Zeile/Angabe **weglassen** (keine `[Platzhalter: …]` im ausgelieferten Dokument), offener Punkt intern (ClickUp/Chat) getrackt. On-Creative-Pflichtwerte (Disclaimer, Offer-Preis) ohne Klärung → Konzept zurückhalten.
- **Build-fertig & zeilenweise (Dokument-Disziplin).** Ein Briefing ist eine **Bau-Anweisung**, kein Strategie-Essay. **Ausführer-Test für jeden Block: braucht der Designer/Editor/Creator das, um das Asset zu bauen? Nein → raus.** Keine Callouts vor der Arbeit (kein todo/anchor/locked), kein Intro-Absatz, kein `h1 "Briefing"` - Strategie steckt in **einer `Idee`-Zeile** (1–2 Zeilen), offene Punkte leben in ClickUp/Chat. Jede Aufzählung/Anweisung steht **zeilenweise** (nie als „1) … 2) … 3) …"-Block). Build-Zellen sind **final entschieden** (keine offenen Fragen, kein „evtl."), **ein konkreter Fakt statt Floskel** (keine Badge-Doppler). **Rollen statt Namen** - keine Personennamen im Dokument. (UGC-Briefing: Kunden-Layer erlaubt, aber hinten als Anhang - Creator-Teil zuerst.) Struktur & Skelette: `references/document-format.md`.
- **„Auf einen Blick" + Format-Regel (Designer-Standard).** Jedes Designer-Briefing trägt oben **Ziel · Zielgruppe · Idee · Formate · Deadline · Assets · Abgabe** (+ „Wichtige Infos" nur, wenn konkret; Motion/Video zusätzlich **Deliverables-Zählung** und **VO & Captions**) - **nur Zeilen mit echter Info**: fehlende Werte im Intake nachfragen, sonst Zeile weglassen. Die **Assets-Quelle ist Pflicht-Rückfrage** (Logo/Fonts/Packshots/Footage/Referenzen-Link - der häufigste Produktionsblocker). **Keine Legende im Dokument** (die Bold-Konvention wird nicht erklärt, keine 1:1-Übernahme-Anweisung - Creatives dürfen Wordings challengen; gesperrte Texte sichert Autoren-Standard + Abnahme). **Fett = landet auf dem Creative, und NUR das** (nie fett: #, Konzept-Name, Dateiname, Visual-Direction). **Kein Ads-Manager-CTA im Briefing** (der wird in Meta eingestellt); CTA nur als gestaltetes On-Creative-Element. **Meta-Formate: 4:5 + 9:16** (im Dokument nur diese Zeile; Kein-1:1-/Sonderformat-Hinweise nur bei echtem Anlass). **Kein „·" als Trenner im Dokument-Inhalt** (dek/Zellen: Kommas oder Zeilen). Je Static/Konzept das **Creative Format** (Ad-Style aus `references/creative-formats.md`) benennen. **Visual-Direction so weit wie möglich reduzieren - Default leer:** die Spalte bleibt, aber nur füllen, wenn das Konzept eine visuelle Richtung zwingend braucht oder es eine klare Kunden-Vorgabe gibt; sonst leer (Designer entscheidet die Optik). Keine Detail-Beschreibung, keine Stimmungswörter, keine Formate/Farben.
- **Advantage+/KI-Awareness (2026).** Im Briefing festlegen, welche Elemente **KI-invariant** sind (Logo, Claim, gesperrte Zahlen, Farben) - Metas Auto-Enhancements können Text/Claims verändern (Vorschau vor Launch prüfen). Extern KI-generierte, fotorealistische Assets als solche kennzeichnen (Metas AI-Disclosure-Anforderungen; Details + Primärquellen: `admkrs-cs-ad-compliance-check`). Details: `references/field-notes.md` §5.

### Harte Sicherheits-Regeln (nicht verhandelbar)
- **Nie senden / posten / publizieren / hochladen** (E-Mail, Slack, ClickUp, Ads Manager, Social) ohne explizite Freigabe. Alles geht als **Entwurf** raus.
- **Nie löschen** (Dateien, Tasks, Docs) ohne explizite Freigabe. Verschieben in Archiv ist ok.
- **Sprache spiegeln:** in der Sprache des Users antworten; Briefing-Sprache = Zielmarkt-Sprache (Beispiel: NL-Markt → Englisch/Niederländisch je nach Vorgabe, DACH → Deutsch).

---

## Workflow

Arbeite diese Schritte ab. Wenn du genug Kontext hast, **fang an** und zeig früh einen Stand - frag nur, wenn ohne Klärung ein wesentlicher Teil der Arbeit für die Tonne wäre.

### 1 - Briefing-Typ bestimmen
Welcher Output ist gewünscht? (Mehrere möglich.)

| Typ | Wann | Dokument-Skelett |
| --- | --- | --- |
| **Statics** | Single-Image-Ads, Reviews, Comparison, Offer | **„Auf einen Blick"** → Tabelle `[# | Static | Dateiname | Produkt | Creative Format | Hook | Subline | USPs/Badge | CTA | Disclaimer | Visual-Direction]` (1 Zeile/Static) |
| **Carousel** | Multi-Card-Storytelling, Edukation, Range | wie Statics → Carousel-Tabelle (1 Zeile/Card) |
| **Motion** | animierte Ads, kinetische Typo, keine Live-Faces | **„Auf einen Blick"** (inkl. Deliverables + VO & Captions) → pro Konzept: keyvalue (Idee·Length·Offer·VO·Musik) + 3 Hooks + VO-Spine + time-coded Storyboard (letzte Zeile = End-Card) |
| **Video / UGC** | Talking-Head, UGC, Studio, VSL | wie Motion, Storyboard heißt **Script** (time-coded); Assets-Zeile inkl. **Rohmaterial-Link** |
...
Prüfe den Brief gegen die Checkliste in `references/intake-questions.md`. **Nicht alles abfragen** - nur die Lücken, die das Briefing sonst falsch machen würden. Wenn der Brief widersprüchlich oder löchrig ist, **sag es, bevor du schreibst** (z. B. Offer fehlt, Zielgruppe unklar, Claim nicht freigegeben). In Cowork: nutze das Frage-Tool, batche Fragen, halte sie kurz.

### 4 - Strategie-Pass
Bevor du textest: diagnostizieren und Architektur bauen. **Wenn der Kunde in DatAds liegt, zuerst die Performance-Signale ziehen** (`references/performance-playbook.md`: Bestperformer, Best Hooks, Best Ad Copy, Ad Angles, Ads to Kill) - briefe mehr von dem, was gewinnt, iteriere Gewinner, vermeide „Ads to Kill". Dann mit `references/creative-strategy.md` (Angles/Awareness), `references/creative-methods.md` (Testing, Diagnose, Video-/UGC-Mechanik) und `references/field-notes.md` (verifizierte 2026-Frameworks: P.D.A./Concept-Architecture, Ad Families) entscheiden:
- Awareness-Stage / Audience-Temperatur (cold/warm/hot)?
...
- **Voss-Schärfe festlegen** (Mut-Grad der Haltung): **1** = Premium-safe (klare Haltung, kein Anecken) · **2** = mutig (Default) · **3** = voll Voss (polarisierend). Wird im Strategie-Pass (Chat) festgehalten. **Stufe 3 immer durch `admkrs-cs-ad-compliance-check`** (Personal-Attributes-/Backfire-Risiko). Stufen & Muster: `references/voss-principles.md`.
Das Ergebnis dieses Passes lebt im **Chat/intern** - ins Dokument geht nur die **`Idee`-Zeile** (1–2 Zeilen je Set bzw. je Motion-/Video-Konzept).
...
- Visual-Direction nur, wenn das Konzept sie zwingend braucht - sonst leer lassen, der Designer entscheidet die Optik.

### 6 - Dokument bauen
Inhalte in eine Briefing-JSON gießen und mit dem Builder rendern. **Format, Schema, Design-Tokens und der genaue Befehl stehen in `references/document-format.md`.** Nutze die zwei Vorlagen in `examples/` (Statics + Motion) als Muster - Struktur kopieren, Inhalt ersetzen. Builder kurz:

```bash
cd <skill>/assets && npm install docx   # einmalig
node <skill>/assets/build_briefing.js <briefing.json> <YYMMDD_BRAND_Product_Type_LANG.docx>
```
...
- [ ] **Zeilen-Test:** Aufzählungen & mehrere Anweisungen **zeilenweise** (`\n`) statt Block - kein „1) … 2) … 3) …"-Absatz in einer Zeile?
- [ ] **Auf-einen-Blick-Test:** Stehen Ziel, Zielgruppe, Idee und Formate oben im Dokument? (Motion/Video: + Deliverables-Zählung + VO & Captions?) Deadline/Assets/Abgabe **konkret oder weggelassen** - kein `[Platzhalter: …]`, keine Leerformeln? Wurden fehlende Werte im Intake nachgefragt?
- [ ] **Schema-Test:** Entspricht die Tabelle dem **aktuellen Standard-Schema** - auch wenn ein älteres Briefing als Vorlage diente? `#`-Spalte = **nur die Ziffer** (Name in Spalte 2)? **Kein Angle-Label** („Pain", „Objection") in der Tabelle?
- [ ] **Bold-Test (beide Richtungen):** Ist **alles, was aufs Creative soll, fett - und NICHTS anderes**? (`#`, Konzept-Name, Dateiname, Creative Format, Visual-Direction: nie fett.) **Keine Legende/Meta-Anweisungen** im Dokument?
- [ ] **On-Creative-Test:** Steht in den Build-Zellen **nur, was wirklich aufs Creative kommt** (+ Visual-Direction)? **Kein Ads-Manager-CTA** („Jetzt einkaufen"-Button aus Meta) und **keine Freigabe-Marker** (`[CS-Freigabe]`) in der Tabelle?
- [ ] **Text-Budget-Test:** Reichen **Hook + Subline**? USPs nur, wo das Konzept sie braucht - und dann **max. 3 Punkte à 2–4 Wörter** (oder 1 kurzer Satz)? **On-Creative-CTA (2–4 Wörter) gesetzt, wo er dem Creative hilft** (DR/Offer/Hero)? Kein Creative mit USP-Liste UND Zusatz-Sätzen UND Badges gestapelt?
- [ ] **Format-Test (Meta):** 4:5 + 9:16 angegeben (nur die Meta-Standard-Zeile, keine Kein-1:1-/Sonderformat-Hinweise ohne Anlass)? Kein 1:1 produziert? Keine Formatangaben in Visual-Direction-Zellen? Kein „·" als Trenner im Dokument-Inhalt?
- [ ] **Style-Test:** Trägt jedes Static/Konzept ein benanntes **Creative Format** aus der Style-Bibliothek (`creative-formats.md`)?
- [ ] **Visual-Reduktions-Test:** Visual-Direction-Zellen **standardmäßig leer** - gefüllt nur, wo das Konzept es zwingend braucht oder eine Kunden-Vorgabe existiert? Keine Detail-/Stimmungs-Beschreibung, keine Farb-/Format-Vorgaben?
- [ ] **Ausführer-Test (Static/Motion/Video):** Besteht **jeder** Block die Frage „braucht der Ausführer das zum Bauen?" - **keine Callouts** (todo/anchor/locked), kein Intro-Absatz, kein `h1 "Briefing"`, kein Family-Label, kein Target-Audience pro Konzept, kein outro? Strategie = **eine `Idee`-Zeile** (1–2 Zeilen)? (UGC: Kunden-Layer nur hinten als Anhang.)
- [ ] **Final-statt-offen-Test:** Sind Build-Zellen **entschieden** (kein „evtl./oder", keine offene Frage)? **Kein `[Platzhalter: …]` im Dokument** - fehlende Werte nachgefragt und sonst weggelassen, offene Punkte intern getrackt? End-Card als letzte Storyboard-Zeile (Motion/Video)?
- [ ] **Namen-Test:** **Keine Personennamen** im Dokument - Rollen statt Namen („zurück an ADMKRS", nie „[Name] fragen")? (Review-Zitate als On-Creative-Copy ausgenommen.)
...
- `references/creative-strategy.md` - Andromeda-Era-Strategie, Angle-Library, Konzept-Portfolio, Awareness-Stages, Test-Architektur. **Lesen in Schritt 4.**
...
- `references/brand-profile-template.md` - Vorlage für Kunden-Profile + Locked-Facts-Disziplin. **Lesen in Schritt 2, wenn ein Profil angelegt wird.**
