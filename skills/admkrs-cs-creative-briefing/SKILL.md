...
- **Nichts erfinden.** Zahlen, Health Claims, Testimonials, Preise, Offers werden nie erfunden. Was gesperrt/freigegeben ist, bleibt 1:1. Neue Vorschläge werden im Chat/intern als **[Ergänzung]** zur Freigabe gestellt - nicht im Designer-Dokument. Fehlt ein Wert: **`[Platzhalter: …]`** direkt an der betroffenen Stelle - nie raten; der offene Punkt wird intern (ClickUp/Chat) getrackt.
...
- **„Auf einen Blick" + Legende + Format-Regel (Designer-Standard).** Jedes Designer-Briefing trägt oben **Ziel · Zielgruppe · Idee · Formate · Deadline · Assets · Abgabe** (+ „Wichtige Infos" nur, wenn konkret; Motion/Video zusätzlich **Deliverables-Zählung** und **VO & Captions**). Die **Assets-Zeile ist Pflicht** (Logo/Fonts/Packshots/Footage/Referenzen-Link - der häufigste Produktionsblocker). Direkt über der Tabelle die **3-Zeilen-Legende**: Fett = landet auf dem Creative · Alle Texte 1:1 übernehmen · [Platzhalter] = nicht selbst füllen. **Fett = landet auf dem Creative, und NUR das** (nie fett: #, Konzept-Name, Dateiname, Visual-Direction). **Kein Ads-Manager-CTA im Briefing** (der wird in Meta eingestellt); CTA nur als gestaltetes On-Creative-Element. **Meta-Formate: 4:5 + 9:16, kein 1:1**; Sonderformate extra mit Plattform + Pixeln. Je Static/Konzept das **Creative Format** (Ad-Style aus `references/creative-formats.md`) benennen. **Visual-Direction so weit wie möglich reduzieren - Default leer:** die Spalte bleibt, aber nur füllen, wenn das Konzept eine visuelle Richtung zwingend braucht oder es eine klare Kunden-Vorgabe gibt; sonst leer (Designer entscheidet die Optik). Keine Detail-Beschreibung, keine Stimmungswörter, keine Formate/Farben.
...
| --- | --- | --- |
| **Statics** | Single-Image-Ads, Reviews, Comparison, Offer | **„Auf einen Blick"** → Legende → Tabelle `[# | Static | Dateiname | Produkt | Creative Format | Hook | Subline | USPs/Badge | CTA | Disclaimer | Visual-Direction]` (1 Zeile/Static) |
| **Carousel** | Multi-Card-Storytelling, Edukation, Range | wie Statics → Carousel-Tabelle (1 Zeile/Card) |
| **Motion** | animierte Ads, kinetische Typo, keine Live-Faces | **„Auf einen Blick"** (inkl. Deliverables + VO & Captions) → Legende → pro Konzept: keyvalue (Idee·Length·Offer·VO·Musik) + 3 Hooks + VO-Spine + time-coded Storyboard (letzte Zeile = End-Card) |
...
- [ ] **Auf-einen-Blick-Test:** Stehen Ziel, Zielgruppe, Idee, Formate, Deadline, **Assets** und Abgabe oben im Dokument? (Motion/Video: + Deliverables-Zählung + VO & Captions?) Deadline und Assets-Link konkret oder als `[Platzhalter]`?
...
- [ ] **Bold-Test (beide Richtungen):** Ist **alles, was aufs Creative soll, fett - und NICHTS anderes**? (`#`, Konzept-Name, Dateiname, Creative Format, Visual-Direction: nie fett.) **3-Zeilen-Legende** über der Tabelle drin?
...
- [ ] **Final-statt-offen-Test:** Sind Build-Zellen **entschieden** (kein „evtl./oder", keine offene Frage)? Fehlende Werte als `[Platzhalter: …]` **inline an der Stelle**, offene Punkte intern getrackt (nicht im Dokument)? End-Card als letzte Storyboard-Zeile (Motion/Video)?
