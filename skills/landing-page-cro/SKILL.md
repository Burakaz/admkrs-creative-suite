...
description: >
  ...
  Evidence-based (Baymard, NNGroup, Google web.dev). Pairs with creative-briefing
  und performance-reporting (Diagnose CTR-hoch/CVR-niedrig = LP).
---
...
## Wann nutzen
Diagnose „hohe CTR + niedrige CVR" (aus `performance-reporting`) · neue Kampagne braucht eine passende LP · bestehende LP/PDP auditieren · Advertorial vs. dedizierte LP vs. PDP entscheiden.
...
## 6 — Seitentyp wählen
**Dedizierte LP** für Cold-Paid (keine Exits außer Conversion). **PDP** eher für warm/Intent. **Advertorial** (redaktionelle Pre-Sell-Seite) für High-Consideration/Supplements/skeptische Audiences — wärmt vor und entschärft Claim-/Compliance-Risiko (→ `ad-compliance-check`). Kontext-abhängig, keine Universal-Regel.
...
`performance-reporting` (Diagnose CTR↑/CVR↓ → LP) · `creative-briefing` (LP muss Hook spiegeln) · `offer-promo-strategy` (Offer-Darstellung) · `ad-compliance-check` (Advertorials/Claims).
