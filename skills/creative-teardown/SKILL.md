...
description: >
  ...
  Swipe-File-System. Pairs with creative-strategy-os (Audit) und creative-briefing.
---
...
6. **Congruency** — CTA klicken: Hält die Landingpage das Hook-Versprechen? Bruch = Conversion-Killer (und bei Konkurrenz: ausnutzbare Schwäche).

Für die strukturierte Ausgabe: `assets/templates/teardown-template.md`. (Format-/Hook-Taxonomien tiefer in `creative-briefing` → `references/field-notes.md`.)
...
Marken halten verlierende Ads selten lange — **Laufzeit ist das beste öffentliche Signal**. 5 Signale, je 1–3 Punkte (max 15); **9+ = wahrscheinlicher Winner zum Remixen** (HeyOz-System):
| Signal | 1 | 2 | 3 |
| --- | --- | --- | --- |
| Laufzeit | <14 T | 14–30 T | 30+ T |
| Varianten | 1 | 2–4 | 5+ |
| Länder | 1 | 2–4 | 5+ |
| Überlebt seit | neu | 2+ Wo | 4+ Wo |
...
`creative-strategy-os` (Audit/Bottleneck) · `creative-briefing` (Teardown → Briefing) · `pitch-teardown` (Teardown als Sales-Asset) · `ad-compliance-check` (bevor man fremde Hooks adaptiert).
