...
- **Dokument-Skelett (schlank, build-fertig - keine Callouts, kein Intro, kein `h1 "Briefing"`):** **„Auf einen Blick"** (Ziel · Zielgruppe · Idee · Formate · Deadline · **Assets** · Abgabe · „Wichtige Infos" nur wenn konkret) → **3-Zeilen-Legende** (Fett = Creative · Texte 1:1 · [Platzhalter] nicht füllen) → Briefing-Tabelle `[# | Static | Dateiname | Produkt | Creative Format | Hook | Subline | USPs / Badge | CTA | Disclaimer | Visual-Direction]` (**eine Zeile pro Static**, Zellen zeilenweise; #-Spalte NUR die Ziffer, Name in Spalte 2, kein Angle-Label wie „Pain" in der Tabelle. **Subline, USPs, CTA, Disclaimer sind optional und bleiben leer (`–`), wenn sie keinen eigenen Job machen** - so wenig Text aufs Creative wie möglich; optionale Spalten ganz weglassen, wenn im ganzen Set leer. **Gilt auch beim Fortschreiben älterer Briefings: Standard schlägt Vorlage.**) → optional `h3 "Sternchentexte im Wortlaut"` UNTER der Tabelle (nur Langtexte). Offene Punkte leben in ClickUp/Chat, im Dokument nur `[Platzhalter: …]` inline; **keine Personennamen** (Rollen statt Namen). **Keine Strategie-Layer-/Hebel-Tabelle** im Designer-Dokument. Carousel: letzte Tabelle = `[Card | Visual | On-Card-Text | Zweck]`, Card 1 = Standalone-Hook, letzte Card = CTA.
...
**So wenig Text wie möglich, Klarheit zuerst.** Der Hook trägt, oft reicht Hook + Produkt. Jede weitere Zeile (Subline, USPs, CTA, Disclaimer) muss sich ihren Platz **verdienen** (eigener Job, kein Wiederholen) - sonst bleibt sie leer. Subtraktions-Test pro Zeile: „Versteht man es auch ohne? Doppelt sie etwas? → weg." CTA meist über den Meta-Button, nicht aufs Creative; Disclaimer nur wenn ein Claim ihn braucht (dann als kleine Fußzeile, Länge ok).
...
<sub>**ADMKRS Creative Suite** · © ADMKRS GmbH, München · v1.11.0 · interner Gebrauch · Format-Tür zu `admkrs-cs-creative-briefing` (gemeinsame Engine, keine Duplikate).</sub>
