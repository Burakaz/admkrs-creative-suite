...
Verboten: nicht-typische/garantierte Outcomes (Health/Weight/Beauty/Finance), Einkommensgarantien/„kündige deinen Job", **Fake-Urgency** („nur noch 3!" wenn unwahr), irreführende Vergleiche, **Ad-Claim ≠ LP** (eigener Ablehnungsgrund → `landing-page-cro`).
...
Prüft Output von `creative-briefing`, `ugc-briefing` (Hooks/Scripts/Kennzeichnung), `ugc-creator-ops` (Disclosure), `offer-promo-strategy` (Preis-/Rabatt-Auslobung), `landing-page-cro` (Ad↔LP-Match). Google-Policies separat (→ `google-cross-channel`).
