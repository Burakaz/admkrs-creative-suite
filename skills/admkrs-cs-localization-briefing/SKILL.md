...
1. **Struktur auf Englisch** (Labels, Header, Spalten, Notizen) - internationale Teams arbeiten daran. Nur die **Copy-Zellen** tragen **Original (DE)** + **Übersetzung (Zielsprache)**.
...
## Vorgehen
1. **Intake & Flag.** Quell-Copy (DE, freigegeben), Zielsprache, Glossar (bindend), Quell-Design-Files (DE-Master: Figma/PSD, bei Motion AE-/Schnitt-Projekt), Deadline, Abgabeort + Export-Naming, Anrede-Register pro Markt (je/u, tu/vous, tu/usted); optional Native-/Kunden-Feedback und Markt-Compliance-Regeln. Widersprüche/Lücken im Quell-Briefing melden, bevor übersetzt wird (fehlender Disclaimer, Claim nicht freigegeben, Offer unklar). Nicht raten.
...
4. **Dokument bauen.** Über `../admkrs-cs-creative-briefing/assets/build_briefing.js` (englische Labels; „At a glance"-Block mit Scope, Source files, Formats, Deadline, Delivery; Statics-Vergleichstabelle `Asset | Element | Original (DE) | Translation (LANG)` / Motion 2-Spalten `On-Screen Text | Voice-Over`). Muster: `examples/`. Format-Regeln: `references/document-format.md`.
...
<sub>**ADMKRS Creative Suite** · © ADMKRS GmbH, München · v1.13.0 · interner Gebrauch · Sibling zu `admkrs-cs-creative-briefing` (gemeinsame Engine, keine Duplikate).</sub>
