...
description: >
  ...
  AI-supported and pairs with the `creative-briefing` skill (which writes the
...
> Companion: `creative-briefing` schreibt die eigentlichen Briefings (Hooks, Konzepte, Storyboards, docx). Dieser Skill sagt dir, *wann* und *warum* du briefst, *was* du testest und *wie* du liest. Beide zusammen = volle Creative-Strategy-Funktion.
...
**AI-Support:** DatAds; `creative-briefing/references/performance-playbook.md`. Details: `references/onboarding-and-audit.md`.
...
**AI-Support:** AI zum Scrapen/Clustern der VoC; `creative-briefing` references `creative-strategy.md` (Angle-Library) + `field-notes.md` (P.D.A.). Details: `references/research-diversity-testdesign.md`.
...
**AI-Support:** AI generiert die Matrix + Varianten-Optionen; `creative-briefing` schreibt die Briefs. → Vorlage: `assets/templates/concept-matrix.md`.
...
- **Briefs** mit `creative-briefing` (eine Briefing-Datei = die Konzepte der Runde). **Alles benennen:** `Format_Hook_Angle_Audience`.
...
**AI-Support:** `creative-briefing`; AI-Bild/Video-Gen (SLCT/Pass³ aus `field-notes.md`).
...
**AI-Support:** `creative-briefing` Iterationen; DatAds.
...
- Format-Bibliothek (alle Video-/Static-Styles + Einsatzzweck + Style-Decoder): siehe `creative-briefing` → `references/creative-formats.md`.
