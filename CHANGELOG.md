...
## v1.13.0 - 2026-07
Das grosse Qualitäts-Release: neuer Skill **`admkrs-cs-ad-psychology`**, Briefing-Standard-Update (keine Legende, keine Platzhalter), Suite-weiter Qualitäts-Audit mit 131 eingearbeiteten Fixes, Tutorial-Umbau, Kundendaten aus dem Repo entfernt.

**Neuer 17. Skill `admkrs-cs-ad-psychology` (Kaufpsychologie):** das Messy-Middle-Modell (Google/The Behavioural Architects 2020, 310.000 simulierte Käufe) plus die 6 wirksamsten Kauf-Biases mit Effektstärken (Social Proof top in 28/31 Kategorien; Scarcity meist am schwächsten und im Cold Prospecting kontraproduktiv), übersetzt in Explore-/Evaluate-Messaging, Bias-Schnellwahl-Matrix und On-Creative-Elemente. Kern-Prinzipien: Präsenz schlägt Perfektion, Sicherheit schlägt Überlegenheit (Evaluation = Reassurance-Job), Marke bleibt der Anker, Nudge statt Sludge, ein Bias-Element pro Creative. Simulations-Evidenz als direktional gekennzeichnet.

**Briefing-Standard-Update (alle Briefing-Skills + Beispiele):** (1) Keine `[Platzhalter: …]` mehr im ausgelieferten Dokument: Pflicht-Rückfragen im Intake (Deadline, Assets, Abgabe, On-Creative-Pflichtwerte); bleibt ein Wert offen, wird die Zeile weggelassen bzw. das Konzept zurückgehalten. (2) Die 3-Zeilen-Legende entfällt ersatzlos, keine "1:1 übernehmen"-Anweisung: Creatives dürfen freie Wordings challengen, gesperrte Claims/Zahlen sichert Autoren-Standard + Verifier-Abnahme. (3) Im Dokument steht nur die Meta-Standard-Formatzeile 4:5 + 9:16 (Kein-1:1-/Sonderformat-Hinweise nur bei echtem Anlass). (4) Kein Mittelpunkt-Trenner im Dokument-Inhalt (Kommas/Zeilen; Überschriften mit Doppelpunkt), bessere Kompatibilität beim Übernehmen in andere Programme.

**Qualitäts-Audit über alle Skills, Website und Tutorial (18 Reviewer, 118 Findings, 131 Fixes):** u.a. Compliance-Skill mit Primärquellen verankert (Meta Transparency Center, PAngV §11/EU-Omnibus 30-Tage-Bestpreisregel, DSA Art. 26/28, Vorab-Genehmigungs-Kategorien, "Wenn doch abgelehnt"-Block) und Fake-Präzisions-Statistiken entfernt; alter Strategie-Layer-Standard aus creative-strategy.md §8 getilgt; Pitch-Teardown mit Gap-Quantifizierungs-Methode (keine EUR-Zahl vor Discovery, nur Prospect-Inputs als Range); Andromeda-Entity-Behauptungen als Praktiker-Interpretation gekennzeichnet; unbelegte Vendor-Zahlen suite-weit qualitativ umformuliert oder mit Quellen belegt; Hook-Bold-Konvention in allen Beispielen vereinheitlicht (alle 3 Hooks fett); Builder-Aufrufe repariert.

**Tutorial-Umbau (docs/prozess.html):** kanonisches Stationsmodell synchron zu flow.html (Schritt 0 Pitch + Stationen 1-8, inkl. neuer Stationen Produktion und Abnahme), Rollen-Schnellstart (6 Rollen mit klickbaren Stationen), NOVA als durchgängiger Beispiel-Fall pro Station, Kill/Scale-Regeln + "Wann darfst du urteilen?"-Box, Test-Setup-Punkt, kopierbare Start-Prompts pro Station.

**Natürlicher Sprechfluss für UGC/VO (neue Doktrin, `creator-script-craft.md` §5b):** Scripts in echter gesprochener Sprache statt Schriftdeutsch: Diskursmarker und Füllwörter sind erwünscht ("also", "ehrlich", "halt"), Kontraktionen, Überleitungen zwischen Beats, Nachträge, Selbstkorrekturen - an Füllwörtern und Überleitungen wird nicht gespart (Kürze-Doktrin gilt für On-Screen-Text, nicht fürs Gesprochene). Pflicht-Gate: jede Zeile laut sprechen ("Würde ich das SO zu einer Freundin sagen?"). Ad-Marker-Floskeln banned ("Hör auf zu scrollen", "Du brauchst dieses Produkt"). **Paid-CTA-Regel suite-weit: nie "Link in Bio"** (Organik-Sprache), stattdessen "klick einfach aufs Video" / "tipp unten drauf" - in creator-script-craft, copywriting.md §5, creative-formats.md und allen Beispielen umgesetzt. Quellen: Paul Graham "Write Like You Talk", Broadcast-Writing-Prinzipien (CWI Pressbooks), eLearning Coach VO-Guide, Linguistik gesprochener Sprache (Wikipedia/IDS).

**Kundendaten-Bereinigung:** die reale Kundenkarte aus dem öffentlichen Repo entfernt (liegt jetzt intern) und die Git-History per Rewrite vollständig bereinigt (Datei + Namens-Erwähnungen aus allen alten Commits getilgt). Im Repo liegt nur noch die fiktive Struktur-Vorlage `nova.md`; Kundenkarten werden dem Verifier zur Laufzeit mitgegeben. FAQ/README entsprechend präzisiert. Verifier-Designcheck-Zahl auf 49 Punkte korrigiert.

**Website ausgelagert:** die Team-Website (Guide, Ablauf, Tutorial, Style-Katalog) liegt nicht mehr als `docs/` in diesem Repo, sondern auf **https://suite.admkrs.com** (Vercel), zugänglich nur mit Google-Login einer @admkrs.com-Adresse. GitHub Pages ist abgeschaltet. Der Styles-Katalog-Generator schreibt direkt ins Vercel-Projekt.
...
**Website, neue Seite „Ablauf" (`docs/flow.html`):** eine zoom- und schiebbare Vollbild-Canvas-Map des gesamten Workflows. Acht Stationen als eine Linie von links nach rechts in drei Phasen (Strategie · Umsetzung · Launch & Lernen), der Pitch als einmaliger Einstieg davor, der Flywheel-Loop dreht unten sauber zurück zum Anfang. Klick auf eine Station öffnet ein Detail-Panel (Inhalte blenden gestaffelt von rechts ein) mit den greifenden Skills; jede Skill-Karte klappt auf und zeigt, was der Skill macht und worin er stark ist. Canvas randlos bis zur Menüleiste, „Ablauf" in der Navigation.

**`admkrs-cs-static-briefing` schärfer auf Text-Minimierung:** neue Leit-Doktrin „so wenig Text wie möglich, Klarheit zuerst". Der Hook trägt, oft reicht Hook + Produkt. Subline, USPs, CTA und Disclaimer sind optional und bleiben leer, wenn sie keinen eigenen Job machen (Subtraktions-Test pro Zeile). CTA meist über den Meta-Button statt aufs Creative; wichtige Disclaimer dürfen lang sein (kleine Fußzeile). Das Static-Beispiel als dopplungsfreie, schlanke Vorlage neu gebaut (u. a. ein Static nur mit Hook). Geregelt in `document-format.md` (Text-Budget) und `copywriting.md` §5.

**Em-Dash-Hausregel hart verankert:** das Zeichen „—" repo-weit entfernt (Skills, Referenzen, Templates, Website, README) und als Schreibregel überall gesetzt. Klingt nach AI, hat in ADMKRS-Output nichts verloren.

**Plugin/Marketplace:** `description` auf 15 Skills aktualisiert und Em-Dash-frei.
